...
This document describes all configuration options and settings available in the TaskerHA Companion custom integration.
...
### `taskerha_companion.example_service`
...
```yaml
service: taskerha_companion.example_service
target:
  ...
  - alias: "Call service at sunset"
    trigger:
      - trigger: sun
        event: sunset
    action:
      - action: taskerha_companion.example_service
        target:
          entity_id: switch.device_name_switch
        data:
          parameter: "mode"
  ...
  input:
    sensor_entity:
      ...
      selector:
        entity:
          domain: sensor
          integration: taskerha_companion
    threshold:
      name: Threshold
      selector:
        number:
...
- [GitHub Issues](https://github.com/db1996/taskerha-hacs/issues) - Report problems
